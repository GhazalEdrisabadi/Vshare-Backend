...
class UserNotification(ListAPIView):
	...
	def get_queryset(self):
		...
		try:
			notifications = Notification.objects.filter(receiver=user, is_seen=False).exclude(notification_type=1).exclude(notification_type=2)
			...
			if notifications:
				for notify in notifications:
					notify.is_seen = True
				notify_list = notifications.union(friend_count_notify, group_count_notify)
				return notify_list
			...
			# There is no new notification
				...
				notify_list = last_10_notifications.union(friend_count_notify, group_count_notify)
				return notify_list
